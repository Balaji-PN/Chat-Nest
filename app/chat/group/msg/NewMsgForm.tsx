  ...
  const handleSubmit = async () => {
    ...
    for (const fileObj of selectedFiles) {
      const { data, error } = await supabase.storage
        .from("test")
        .upload(`${groupId}/${fileObj.file.name}`, fileObj.file);
    ...
    await axios.post("/api/group/msg", {
      content,
      groupId,
        ...
        <ScrollArea className="w-full rounded-lg border p-3 mb-2">
          <div className="flex w-max space-x-2">
            {selectedFiles.map((fileObj) => (
              <div
                key={fileObj.id}
                className="flex items-center bg-secondary text-secondary-foreground rounded-md pl-2 pr-1 py-1 text-sm shadow-sm border border-gray-200 dark:border-gray-600"
              >
